...
//        callback();
//    }
    [self.healthStore requestAuthorizationToShareTypes:[NSSet setWithObject:stepType] readTypes:[NSSet setWithObject:stepType] completion:^(BOOL success, NSError * _Nullable error) {
        if(success) {
...
- (void)fetchSumOfSamplesTodayForType:(HKQuantityType *)quantityType unit:(HKUnit *)unit completion:(void (^)(double, NSError *))completionHandler {
    ...
    HKStatisticsQuery *query = [[HKStatisticsQuery alloc] initWithQuantityType:quantityType quantitySamplePredicate:predicate options:HKStatisticsOptionSeparateBySource completionHandler:^(HKStatisticsQuery *query, HKStatistics *result, NSError *error) {
...
- (void)writeData {
    [self authIfNeed:^{
        ...
        HKDevice *device = [[HKDevice alloc] initWithName:@"initWithName" manufacturer:@"manufacturer" model:@"model" hardwareVersion:@"hardwareVersion" firmwareVersion:@"firmwareVersion" softwareVersion:@"softwareVersion" localIdentifier:@"localIdentifier" UDIDeviceIdentifier:@"UDIDeviceIdentifier"];

        HKQuantitySample *stepConsumedSample = [HKQuantitySample quantitySampleWithType:stepType quantity:stepQuantityConsumed startDate:startDate endDate:endDate device:device metadata:nil];
